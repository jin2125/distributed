...
public class ChatRoomServer {
    ...
    //相当于自定义协议格式，与客户端协商好
    private static String USER_CONZTENT_SPLIT = "#@#";
    ...
    private void dealWithSelectionKey(ServerSocketChannel server, SelectionKey sk) throws IOException {
        if (sk.isAcceptable()) {
            ...
            // 并注册到selector上。之后这个连接的数据，就由这个SocketChannel处
        ...
        if (sk.isReadable()) {
            ...
            if (content.length() > 0) {
                String[] arrayContent = content.toString().split(USER_CONZTENT_SPLIT);
                // 注册用户
                if (arrayContent != null && arrayContent.length == 1) {
                    ...
                    String name = arrayContent[0];
                    String message = content.substring(name.length() + USER_CONZTENT_SPLIT.length());
